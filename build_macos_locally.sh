#!/bin/bash
set -e

target=darwin-x64
install_name=darwin-x86_64

echo "Configuration Target=${target}, DependencyInstallName=${install_name}"

repo=$(git rev-parse --show-toplevel)
current_dir=$(pwd)
cd $repo


working_dir=build/$target

mkdir -p $working_dir


echo Build dependencies
build_httprequest()
{
    cmake -B$1 -DBUILD_SHARED_LIBS:BOOL=OFF -DCMAKE_POSITION_INDEPENDENT_CODE:BOOL=ON -DCMAKE_INSTALL_PREFIX=$2 -Hthirdparty/cpp-httplib
    make -C$1 -j8
    make -C$1 install
}

if [ ! -d build/external/$install_name ]
then
echo "Generating dependency"
build_httprequest $working_dir/httprequest build/external/$install_name
else
echo "Dependency already generated"
fi

echo Running CMake
cmake -B$working_dir -H.

echo Compiling

make -C$working_dir -j8
make -C$working_dir/oosmc-fmu install
cd $current_dir

echo Done